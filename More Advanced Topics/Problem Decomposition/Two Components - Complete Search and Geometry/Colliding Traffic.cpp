...
double solve(int i, int j){
    double s1 = a[i].s * cos(a[i].d) - a[j].s * cos(a[j].d);
    double s2 = a[i].s * sin(a[i].d) - a[j].s * sin(a[j].d);
    double p = s1*s1 + s2*s2;
    double q = 2*(s1*(a[i].x - a[j].x) + s2*(a[i].y - a[j].y));
    double c = sq(a[i].x - a[j].x) + sq(a[i].y - a[j].y) - sq(r);
    if(q*q - 4*p*c < 0) return INF;
    double ans1 = - q / (2*p) - sqrt(q*q - 4*p*c)/(2*p);
    double ans2 = - q / (2*p) + sqrt(q*q - 4*p*c)/(2*p);
    if(ans2 < 0) return INF;
    return ans1;
}
...
int main(){
    ...
    while(tc--){
        ...
        for(int i=0; i<n; i++){
            ...
            a[i].d = DEG_to_RAD(-(a[i].d - 90.0));
        }
        bool flag = false;
        double max1 = INF;
        for(int i=0; i<n; i++){
            for(int j=i+1; j<n; j++){
                if(hypot(a[i].x - a[j].x, a[i].y - a[j].y) <= r) flag = true;
                max1 = min (max1, solve(i, j));
            }
        }
        if(flag) {
            cout << "0\n";
        }else{
            if(max1 == INF) cout << "No collision.\n";
            else cout << (int)round(max1) << "\n";
        }
    }
    return 0;
}
