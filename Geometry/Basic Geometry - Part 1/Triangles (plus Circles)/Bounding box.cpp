...
int main(){
    ...
    while(cin >> n, n){
        ...
        circumCircle(a, b, c, ctr, r);
        double th1 = atan((a.y - ctr.y)/(a.x - ctr.x));
        double th0 = 2 * PI / n;
        point ans1(inf, inf), ans2(-inf, -inf);
        for(int i=0; i<=n; i++){
            double th = th1 + th0 * i;
            double tx = ctr.x + r * cos(th);
            double ty = ctr.y + r * sin(th);
            ans1.x = min(ans1.x, tx); ans1.y = min(ans1.y, ty);
            ans2.x = max(ans2.x, tx); ans2.y = max(ans2.y, ty);
        }
        cout << "Polygon " << cas++ << ": " << (ans2.x - ans1.x) * (ans2.y - ans1.y) << "\n";
    }
    return 0;
}
